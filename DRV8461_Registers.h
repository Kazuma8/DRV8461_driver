...
enum DRV8461_CTRL3_Reg_Val : uint8_t {
  DRV8461_CTRL3_DIR = 0x80,            // Direction input (when SPI_DIR = 1).
  DRV8461_CTRL3_STEP = 0x40,           // Step input (when SPI_SETP = 1), logic '1' advances one step (self-clearing).
  DRV8461_CTRL3_SPI_DIR = 0x20,        // Direction is controlled by SPI.
  DRV8461_CTRL3_SPI_STEP = 0x10,       // Stepping is controlled by SPI.
  DRV8461_CTRL3_MICROSTEP_MODE = 0x0F, // Micro-stepping mode (default is 0110b = 1/16 step).
};
...
enum DRV8461_CTRL4_Reg_Val : uint8_t {
  DRV8461_CTRL4_CLR_FLT = 0x80,        // Clear all latched fault bits (self-resetting).
  DRV8461_CTRL4_LOCK = 0x60,           // Lock registers and ignore further register writes except for CLR_FLT.
  DRV8461_CTRL4_UNLOCK = 0x30,         // Unlock all registers.
  DRV8461_CTRL4_EN_OL = 0x08,          // Enable open load detection.
  DRV8461_CTRL4_OCP_MODE = 0x04,       // Overcurrent conditon fault latching (0 = latching, 1 = automatic retrying).
  DRV8461_CTRL4_OTSD_MODE = 0x02,      // Overtemperature conditon fault latching (0 = latching, 1 = automatic recovery).
  DRV8461_CTRL4_TW_REP = 0x01,         // Overtemperature warning reporting on nFAULT (0 = not reported, 1 = reported).
};


// CONTROL 5 REGISTER SETINGS ************************************************************************************// 
enum DRV8461_CTRL5_Reg_Val : uint8_t {
  DRV8461_CTRL5_STL_LRN = 0x20,        // Start the stall learning process (automatically resets when complete).
  DRV8461_CTRL5_EN_STL = 0x10,         // Enable stall detection.
  DRV8461_CTRL5_STL_REP = 0x08,        // Stall detection reporting on nFAULT (0 = not reported, 1 = reported).
};


// CONTROL 6 REGISTER SETINGS ************************************************************************************// 
enum DRV8461_CTRL6_Reg_Val : uint8_t {
  DRV8461_CTRL6_STALL_TH = 0xFF,       // Lower 8-btis of stall threshold (default is 00000011b).
};


// CONTROL 7 REGISTER SETINGS ************************************************************************************// 
enum DRV8461_CTRL7_Reg_Val : uint8_t {
  DRV8461_CTRL7_RC_RIPPLE = 0xC0,      // Ripple setting (00b = 1%, 01b = 2%, 10b = 4%, 11b = 6%).
  DRV8461_CTRL7_EN_SSC = 0x20,         // Enable spread-spectrum (on by default).
  DRV8461_CTRL7_TRQ_SCALE = 0x10,      // Apply 8x torque scaling (off by default).
  DRV8461_CTRL7_STL_REP = 0x0F,        // Upper 4-bits of stall threshold.
};
...
enum DRV8461_CTRL8_Reg_Val : uint8_t {
  DRV8461_CTRL8_TRQ_COUNT = 0xFF,      // Lower 8-bits of TRQ_COUNT.
};


// CONTROL 9 REGISTER SETINGS ************************************************************************************// 
enum DRV8461_CTRL9_Reg_Val : uint8_t {
  DRV8461_CTRL9_REV_ID = 0xF0,         // Silicon revision identification.
  DRV8461_CTRL9_TRQ_SCALE = 0x0F,      // Upper 4-bits of TRQ_COUNT.
};
