/*  Vulintus_DRV8461.cpp - copyright Vulintus, Inc., 2023
...
#include <Vulintus_DRV8461.h>          // Library header.
...
Vulintus_DRV8461::Vulintus_DRV8461(SPIClass *spi_bus, uint8_t pin_cs)
    : _pin_cs(pin_cs)
{
...
Vulintus_DRV8461::Vulintus_DRV8461(SPIClass *spi_bus, uint8_t pin_cs, uint8_t pin_slp)
...
// Initialization.
void Vulintus_DRV8461::begin(void)
{
...
// Set the fault pin.
void Vulintus_DRV8461::set_fault_pin(uint8_t pin_flt)
{
...
// uint8_t Vulintus_DRV8461::set_fault_interrupt(void)
// {
...
//     return DRV8461_NO_ERROR;                             // Return the no-error code.
//   }
//   else {                                                  // Otherwise, if no fault pin is set...
//     return DRV8461_ERR_NO_FLT_PIN;                       // Return an error.
//   }
// }


// // Set the fault interrupt (specifying a pin).
// void Vulintus_DRV8461::set_fault_interrupt(uint8_t pin_flt)
// {
//   Vulintus_DRV8461::set_fault_pin(pin_flt);              // Set the fault pin.
//   attachInterrupt(digitalPinToInterrupt(_pin_flt), 
//         fault_interrupt, FALLING);                        // Set a falling interrupt.
// }
...
uint8_t Vulintus_DRV8461::read_fault(void)
{
  fault = read_register(DRV8461_REG_FAULT);    //Read the value of the fault register.
  return fault;                                 //Return the fault value.
}


// Clear all active faults.
void Vulintus_DRV8461::clear_faults(void)
{
  _ctrl_reg_val[3] != DRV8461_CTRL4_CLR_FLT;             // Set the clear fault bit.
  write_register(DRV8461_REG_CTRL4, _ctrl_reg_val[3]);   // Update control register 4.
}		


// Reset all registers to the default settings.
void Vulintus_DRV8461::reset(void)
{
  ...
  write_register(DRV8461_REG_CTRL1, _ctrl_reg_val[0]);   // Control register 1.
  write_register(DRV8461_REG_CTRL2, _ctrl_reg_val[1]);   // Control register 2.
  write_register(DRV8461_REG_CTRL3, _ctrl_reg_val[2]);   // Control register 3.
  write_register(DRV8461_REG_CTRL4, _ctrl_reg_val[3]);   // Control register 4.
  write_register(DRV8461_REG_CTRL5, _ctrl_reg_val[4]);   // Control register 5.
  write_register(DRV8461_REG_CTRL6, _ctrl_reg_val[5]);   // Control register 6.
  write_register(DRV8461_REG_CTRL7, _ctrl_reg_val[6]);   // Control register 7.
}


// Verify the current control register settings.
bool Vulintus_DRV8461::verify(void)
{
  uint8_t ret = read_register(DRV8461_REG_CTRL1) == _ctrl_reg_val[0] &&
                read_register(DRV8461_REG_CTRL2) == _ctrl_reg_val[1] &&
                read_register(DRV8461_REG_CTRL3) == _ctrl_reg_val[2] &&
                read_register(DRV8461_REG_CTRL4) == _ctrl_reg_val[3] &&
                read_register(DRV8461_REG_CTRL5) == _ctrl_reg_val[4] &&
                read_register(DRV8461_REG_CTRL6) == _ctrl_reg_val[5] &&
                read_register(DRV8461_REG_CTRL7) == _ctrl_reg_val[6];
  return ret;
}	


// Set the current set reference pin.
void Vulintus_DRV8461::set_vref_pin(uint8_t pin_vref)
{
...
void Vulintus_DRV8461::set_vref_pwm(uint8_t pwm_val)
{
...
void Vulintus_DRV8461::set_current_max(uint16_t max_milliamps)
{
...
void Vulintus_DRV8461::set_current(uint16_t milliamps)
{
...
uint16_t Vulintus_DRV8461::actual_current(void) {
  ...
  write_register(DRV8461_REG_CTRL1, _ctrl_reg_val[0]);   // Update control register 1.

  return actual;                                          // Return the actual current limit.
}


// void Vulintus_DRV8461::set_open_load_fault_latch(bool enable);		// Enable/disable open load fault latching (default on).
// void Vulintus_DRV8461::set_open_load_detection(bool enable);			// Enable/disable open load detection (default off).
// void Vulintus_DRV8461::set_pwm_toff(DRV8461_PWM_TOFF toff);			// Set the PWM OFF time (default 16 us).
// void Vulintus_DRV8461::set_decay_mode(DRV8461_Decay_Mode mode);		// Set the decay mode (default is smart tune ripple control).
// void Vulintus_DRV8461::set_overcurrent_fault_latch(bool enable);		// Enable/disable overcurrent fault latching (default on).
// void Vulintus_DRV8461::set_overtemperature_fault_latch(bool enable);	// Enable/disable overtemperature fault latching (default latching).
// void Vulintus_DRV8461::set_temperature_fault(bool enable);			// Enable/disable reporting of over/under temperature warnings on nFAULT (default off).
// void Vulintus_DRV8461::set_ripple_current(DRV8461_RC_Ripple ripple);	// Set the ripple current (default is 19mA + 1% of I_trip).
// void Vulintus_DRV8461::set_spread_spectrum(bool enable);				// Enable/disable spread-spectrum (default on).

// void Vulintus_DRV8461::set_torque_scaling(bool enable);		// Enable/disable 8x torque count scaling (default off).
// uint16_t Vulintus_DRV8461::torque_count(void);				// Read the current torque count.

// void Vulintus_DRV8461::set_stall_detection(bool enable);		// Enable/disable stall detection (default is off).
// void Vulintus_DRV8461::learn_stall_count(void);				// Initiate stall count learning.
// void Vulintus_DRV8461::set_stall_fault(bool enable);			// Set reporting of stalls on nFAULT (default on).
// void Vulintus_DRV8461::set_stall_thresh(uint16_t thresh);		// Manually set the stall threshold (0-4095).
// uint16_t Vulintus_DRV8461::get_stall_thresh(void);			// Read the current stall threshold (0-4095).


// Set the direction pin (disables SPI direction-setting).
void Vulintus_DRV8461::set_dir_pin(uint8_t pin_dir)
{
...
void Vulintus_DRV8461::use_SPI_direction(bool enable)
{
  if (enable) {                                           // If we're switching to SPI direction control...
    _ctrl_reg_val[2] |= DRV8461_CTRL3_SPI_DIR;           // Set the DIR step control bit high in control register 3.
  }
  else {                                                  // If we're switching to hardware direction control...
    _ctrl_reg_val[2] &= ~DRV8461_CTRL3_SPI_DIR;          // Set the DIR step control bit low in control register 3.    
  }
  write_register(DRV8461_REG_CTRL3, _ctrl_reg_val[2]);   // Update control register 3.
}


// Set the direction with SPI.
void Vulintus_DRV8461::set_direction(bool dir) {
  if (_ctrl_reg_val[2] & DRV8461_CTRL3_SPI_DIR) {          // If we're using SPI for direction setting...
    _ctrl_reg_val[2] &= ~DRV8461_CTRL3_DIR;                // Clear the current direction bit.    
...
bool Vulintus_DRV8461::get_direction(void) 
{
  bool dir;                                         // Boolean direction.
  if (_ctrl_reg_val[2] & DRV8461_CTRL3_SPI_DIR) {  // If we're using SPI for direction setting...
    dir = _ctrl_reg_val[2] & DRV8461_CTRL3_DIR;    // Grab the value of the DIR bit in control register 3.
  }
  else {                                            // Otherwise, if we're using the DIR input for direction...
...
void Vulintus_DRV8461::set_step_pin(uint8_t pin_step)
{
...
void Vulintus_DRV8461::use_SPI_step(bool enable)
{
  if (enable) {                                                 // If we're switching to SPI step control...
    _ctrl_reg_val[2] |= DRV8461_CTRL3_SPI_STEP;                // Set the SPI step control bit high in control register 3.
  }
  else {                                                        // If we're switching to hardware step control...
    _ctrl_reg_val[2] &= ~DRV8461_CTRL3_SPI_STEP;               // Set the SPI step control bit low in control register 3.    
  }
  write_register(DRV8461_REG_CTRL3, _ctrl_reg_val[2]);         // Update control register 3.
}	
...
// Trigger a step.
void Vulintus_DRV8461::step(void)
{
  if (_ctrl_reg_val[2] & DRV8461_CTRL3_SPI_STEP) {             // If we're using SPI for stepping...
    _ctrl_reg_val[2] |= DRV8461_CTRL3_STEP;                    // Set the STEP bit high in control register 3.
    write_register(DRV8461_REG_CTRL3, _ctrl_reg_val[2]);       // Update control register 3.
  }
...
void Vulintus_DRV8461::set_microsteps(DRV8461_Micostep_Mode mode)
{
  _ctrl_reg_val[2] &= ~DRV8461_CTRL3_MICROSTEP_MODE;     // Clear the microstep setting in control register 3.
...
// Set the enable pin.
void Vulintus_DRV8461::set_enable_pin(uint8_t pin_en)
{
  ...
  _ctrl_reg_val[1] |= DRV8461_CTRL2_EN_OUT;              // Set the enable bit high in control register 2.
  write_register(DRV8461_REG_CTRL2, _ctrl_reg_val[1]);   // Update control register 2.
}


// Use SPI for enabling/disabling (default on).
void Vulintus_DRV8461::use_SPI_enable(bool enable)
{
  if ((enable) && (_hw_en)) {                               // If we're switching to SPI for enabling/disabling...
    _ctrl_reg_val[1] &= ~DRV8461_CTRL2_EN_OUT;             // Set the enable bit low in control register 2.
    write_register(DRV8461_REG_CTRL2, _ctrl_reg_val[1]);   // Update control register 2.
...
// Enable the driver.
void Vulintus_DRV8461::enable(void)
{
  if (!(_ctrl_reg_val[1] & DRV8461_CTRL2_EN_OUT)) {        // If the EN_OUT bit isn't already set high...
    _ctrl_reg_val[1] |= DRV8461_CTRL2_EN_OUT;              // Set the enable bit high in control register 2.
    write_register(DRV8461_REG_CTRL2, _ctrl_reg_val[1]);   // Update control register 2.
  }
...
// Disable the driver.
void Vulintus_DRV8461::disable(void)
{
  if (_hw_en) {                                             // If the hardware enable line is connected...
...
// Set the sleep pin.
void Vulintus_DRV8461::set_sleep_pin(uint8_t pin_slp)
{
...
uint8_t Vulintus_DRV8461::revision_id(void)
{
  uint8_t ret = read_register(DRV8461_REG_CTRL9);      // Read in the current value of control register 9.
...
// Write to a register.
uint8_t Vulintus_DRV8461::write_register(uint8_t reg_addr, uint8_t new_data)
{
...
uint8_t Vulintus_DRV8461::read_register(uint8_t reg_addr)
{
...
void Vulintus_DRV8461::spi_start(void)
{
  digitalWrite(_pin_cs, LOW);     // Set the chip select line low.
  _spi_bus->beginTransaction(SPISettings(DRV8461_SPI_SPEED, MSBFIRST, SPI_MODE1));   // Set the SPI settings for this chip.  
  delayMicroseconds(1);           // Pause for 1 microsecond. 
}


// End an SPI transaction.
void Vulintus_DRV8461::spi_end(void)
{
...
// void Vulintus_DRV8461::fault_interrupt()
// {
//   drv8434s_fault |= DRV8461_FAULT_FAULT;   //Set the fault flag to 1.
// }	